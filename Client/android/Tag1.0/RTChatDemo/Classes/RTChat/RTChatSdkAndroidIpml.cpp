...
{
    cocos2d::JniMethodInfo methodInfo;
    if(!cocos2d::JniHelper::getMethodInfo(methodInfo,CLASS_NAME,"ReqStartRecording","()Z"))
    {
        return false;
    }
    jobject evoe = GetEVOEObject();
    return methodInfo.env->CallObjectMethod(evoe,methodInfo.methodID);
...
{
    cocos2d::JniMethodInfo methodInfo;
    if(!cocos2d::JniHelper::getMethodInfo(methodInfo,CLASS_NAME,"ReqStopRecording","()Z"))
    {
        return false;
    }
    jobject evoe = GetEVOEObject();
...
{
    cocos2d::JniMethodInfo methodInfo;
    if(!cocos2d::JniHelper::getMethodInfo(methodInfo,CLASS_NAME,"ReqStartPlayVoice","(Ljava/lang/String;)Z"))
    {
        return false;
    }
    ...
    jstring jurl;
    if(voiceUrl != NULL)
        jurl = methodInfo.env->NewStringUTF(voiceUrl);

    jobject evoe = GetEVOEObject();
    return methodInfo.env->CallObjectMethod(evoe,methodInfo.methodID,jurl);
}
...
{
    cocos2d::JniMethodInfo methodInfo;
    if(!cocos2d::JniHelper::getMethodInfo(methodInfo,CLASS_NAME,"ReqStopPlayVoice","()Z"))
    {
        return false;
    }
    jobject evoe = GetEVOEObject();
...
{
    cocos2d::JniMethodInfo methodInfo;
    if(!cocos2d::JniHelper::getMethodInfo(methodInfo,CLASS_NAME,"ReqUpdatePower","(JI)Z"))
    {
        return false;
    }
    jobject evoe = GetEVOEObject();
    return methodInfo.env->CallObjectMethod(evoe,methodInfo.methodID,othertempid,(int)powertype);
 }
...
bool RTChatSDKMain::requestAssignMic(uint64_t othertempid)
{
    ...
    cocos2d::JniMethodInfo methodInfo;
    if(!cocos2d::JniHelper::getMethodInfo(methodInfo,CLASS_NAME,"ReqAssignMic","(J)Z"))
    {
        return false;
    }
    jobject evoe = GetEVOEObject();
    return methodInfo.env->CallObjectMethod(evoe,methodInfo.methodID,othertempid);
}
...
JVOEOBSERVER(int,SomeEnterRoom)(JNIEnv* jni,jobject thiz ,jobjectArray jtempidArray,jobjectArray uniqueidArray,jintArray powers,jboolean isNew)
{
  LOGD("---------------------");
  jint size = jni->GetArrayLength(jtempidArray);
  jint* c_powerArray = jni->GetIntArrayElements(powers,NULL);
  ...
  {   
      ...
      std::string uniqueid = cocos2d::JniHelper::jstring2string(jUniqueID);
      bcopy(uniqueid.c_str(), info.uniqueid, sizeof(info.uniqueid));
      info.power = (enPowerType)c_powerArray[i];

      userList->userinfo[i] = info;
  }

  userList->needClearOld = isNew;          
  RTChatSDKMain::sharedInstance().callbackfunc(enNotifySomeEnterRoom, OPERATION_OK, (const unsigned char*)userList, userList->getSize());
   jni->ReleaseIntArrayElements(powers, c_powerArray, 0);
  return 0;
}
...
JVOEOBSERVER(int,RecordingStop)(JNIEnv* jni,jobject thiz ,jlong size)
{
  LOGD("---------------------");
  return 0;
}

JVOEOBSERVER(int,RecordingUploadEnd)(JNIEnv* jni,jobject thiz ,jstring jurl,jint size,jlong duration)
{
  LOGD("---------------------");
  if( jurl != NULL)
  {
    ...
    callbackdata.voicetime = size;
    std::string url = cocos2d::JniHelper::jstring2string(jurl);

    strncpy(callbackdata.urlbuf, url.c_str(), size);
    RTChatSDKMain::sharedInstance().callbackfunc(enRequestRec, OPERATION_OK, (const unsigned char*)&callbackdata, sizeof(StRequestRec));

  }else
  {
    RTChatSDKMain::sharedInstance().callbackfunc(enRequestRec, OPERATION_FAILED, NULL, 0);
  }
  return 0;
}

JVOEOBSERVER(int,RecordingVoiceDownloadBegin)(JNIEnv* jni,jobject thiz)
{
  LOGD("---------------------");
  return 0;
}

JVOEOBSERVER(int,RecordingVoiceDownloadEnd)(JNIEnv* jni,jobject thiz ,jboolean isOk,jlong size)
{
  LOGD("---------------------");
  if(isOk)
  {
    
    RTChatSDKMain::sharedInstance().callbackfunc(enRequestPlay, OPERATION_OK, NULL,0);

  }else
  {
    RTChatSDKMain::sharedInstance().callbackfunc(enRequestPlay, OPERATION_FAILED, NULL, 0);
  }
  return 0;
  
}

JVOEOBSERVER(int,RecordingVoiceIncoming)(JNIEnv* jni,jobject thiz ,jstring url)
{
  LOGD("---------------------");
  return 0;
}

JVOEOBSERVER(int,NotifyPowerUpdate)(JNIEnv* jni,jobject thiz ,jstring jtempid,jint power)
{
  LOGD("---------------------");
  uint64_t tempid = RTChatSDKMain::sharedInstance().convertJstring2Uint64(jtempid); 
  enPowerType thepower = (enPowerType)power;       
  StNotifyUpdatePower callbackdata(tempid, thepower);
  RTChatSDKMain::sharedInstance().callbackfunc(enNotifyUpdatePower, OPERATION_OK, (const unsigned char*)&callbackdata, sizeof(StNotifyUpdatePower));
  return 0;
}

JVOEOBSERVER(int,ResultAssignRet)(JNIEnv* jni,jobject thiz ,jint ret)
{
  LOGD("---------------------");
  enAssignResult result = (enAssignResult)ret;
  StNotifyAssignResult callbackdata(result);
  RTChatSDKMain::sharedInstance().callbackfunc(enNotifyAssignResult, OPERATION_FAILED, (const unsigned char*)&callbackdata, sizeof(StNotifyAssignResult));
  return 0;
}
